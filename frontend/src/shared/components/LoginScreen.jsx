...
export function LoginScreen({ paypalConfigured, onDemoLogin, }) {
    ...
    const login = async () => {
        ...
        setError("");
        if (paypalConfigured) {
            // Full OAuth round-trip through Flask → PayPal Sandbox → callback
            window.location.href = "/api/auth/login";
            return;
        }
        try {
            const u = await api.post("/api/auth/demo");
            onDemoLogin(u);
        }
        catch {
    ...
    return (<div className="canvas-grid anim-fade fixed inset-0 z-[2000] flex items-center justify-center bg-zinc-50 [background-size:24px_24px]">
      <div className="w-[372px] rounded-[18px] border border-zinc-200 bg-white px-[30px] py-[34px] text-center shadow-[0_8px_30px_rgba(0,0,0,0.08)]">
        <div className="mx-auto mb-4 flex size-12 items-center justify-center rounded-xl bg-zinc-900 font-mono text-lg font-semibold text-white">
          S
        </div>
