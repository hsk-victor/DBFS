"""Victor's independent Stocks authentication session."""
import secrets
...
auth_bp = Blueprint("victor_auth", __name__)
USER_KEY = "victor_user"
STATE_KEY = "victor_oauth_state"
...
def login():
    if not paypal.configured():
        return jsonify({"error": "PayPal not configured - use the Stocks demo login"}), 503
...
def callback():
    ...
    if not code:
        return redirect(f"{Config.FRONTEND_URL}?section=Stocks&login=cancelled")
    token = paypal.exchange_code(code)
    user = paypal.userinfo(token)
    user["demo"] = False
    session[USER_KEY] = user
    return redirect(f"{Config.FRONTEND_URL}?section=Stocks")
